...
router.post("/cart", function(req, res, next) {
	...
	mysql(str, [req.body.g_id, req.body.u_id], function(result) {
		if (result.length == 1) {
			...
			mysql("update cart set num=? where g_id=? and u_id=?", [num, req.body.g_id, req.body.u_id], function(results) {
				res.send("成功")
			})
		}else{
            req.body.status=1;
...
router.post('/delCart',function(req,res){
    res.append("Access-Control-Allow-Origin", "*");
    mysql("update cart set status=0 where g_id=? and u_id=?", [req.body.g_id, req.body.u_id], function(results) {
        res.send("成功")
    })
})
