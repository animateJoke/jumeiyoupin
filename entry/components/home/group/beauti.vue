...
<template>
	<div class="beauti">
		<a v-for="(a,index) in getList1" :data-id="index" :href="'#/binfo?id='+index">
			<img :src="a.image" />
...
<script>
	...
	export default {
		...
		computed: {
			getList1() {
				return this.$store.state.beauti
			}
		},
		methods: {
			jiazai() {
				this.$store.state.numBea++;
...
<style scoped>
	...
	.beauti a[data-id] div {
		background: white;
		padding-left: .12rem;
	}
	
	.beauti a[data-id] div span:first-child {
		color: #fe4070;
	}
	
	.beauti a[data-id] div span {
		color: #666;
	}
